...
CSS is built locally with the Tailwind CLI — no CDN, no Vite/React, just a
one-shot purge build (`package.json` + `tailwind.config.js` at repo root).
Node.js/npm are a **build-time-only** dependency; nothing Node-based runs
at request time. Install with `npm install`, build with `npm run build:css`.
See §6a for details and **§12a for what to do after `git pull`.**
...
├── package.json            # npm devDependency (tailwindcss@2.2.19) — build-time only, nothing Node-based runs at request time
├── package-lock.json
...
Since the Tailwind CDN was removed (§6a), a plain `git pull` is no longer
sufficient on its own — `static/tailwind.css` is a **gitignored build
artifact** and will not come down with the pull. Full sequence for
updating an already-installed host (e.g. a second/backup deploy machine):
...
REM Only the first time ever on this host (installs the Tailwind CLI):
npm install

REM Every pull that touched templates/*.html, static/app.js,
REM static/app/*.js, or tailwind.config.js — regenerates static/tailwind.css:
npm run build:css

nssm restart MeterWorklog
```

If you skip `npm run build:css` after a pull that changed which Tailwind
classes are used, the app still starts fine (Flask doesn't know or care),
but pages render with **stale or missing** styling — there's no CDN
fallback anymore. See [SKILL.md §3a](SKILL.md) for the full operational
runbook version of this, including the troubleshooting entry for what an
unbuilt/stale `tailwind.css` looks like in the browser.
